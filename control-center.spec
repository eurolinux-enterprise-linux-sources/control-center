...
%define wacom_tool gnome-wacom-properties
%define wacom_tool_version 3.3.90
...
Source0: http://download.gnome.org/sources/gnome-control-center/2.28/gnome-control-center-%{version}.tar.bz2
Source1: %{wacom_tool}-%{wacom_tool_version}.tar.bz2
Source2: org.gnome.control-center.defaultbackground.policy
Source3: apply-extra-translations
Source4: extra-translations
Source5: mr.po
Source6: custom-LINGUAS
...
# https://bugzilla.redhat.com/show_bug.cgi?id=552713
Patch64: close-fd-leak.patch

# updated translations
# https://bugzilla.redhat.com/show_bug.cgi?id=576066
Patch65: control-center-translations.patch

# https://bugzilla.redhat.com/show_bug.cgi?id=524942
Patch66: bg-xml-in-xdg-dirs.patch
...
# https://bugzilla.gnome.org/show_bug.cgi?id=606793
Patch103: 0001-Disable-tapping-only-for-touchpads-without-a-left-mo.patch

# https://bugzilla.gnome.org/show_bug.cgi?id=610003
Patch102: threads.patch

### gnome-wacom-properties specific patches
Patch500: gnome-wacom-properties-fix-area-calib.patch
Patch501: gnome-wacom-properties-fix-xrandr-version-check.patch
Patch502: gnome-wacom-properties-fix-for-cintiq-without-xrandr.patch
Patch503: gnome-wacom-properties-add-twinview-and-LED-support.patch
Patch504: gnome-wacom-properties-fix-pad-map-button.patch
...
Requires: gnome-settings-daemon-wacom-config-v2
...
pushd gnome-control-center-%{version}
cp %{SOURCE6} .
cp %{SOURCE5} po
...
# Disabled because they break the build
# https://bugzilla.redhat.com/show_bug.cgi?id=576066#c6
%patch65 -p1 -b .translations
%patch66 -p1 -b .bg-xml-in-xdg-dirs
...
%patch101 -p1 -b .touchpad-capabilities
%patch102 -p1 -b .threads
%patch103 -p1 -b .apple-touchpad

%patch7 -p1 -b .make-default
popd

### gnome-wacom-properties
pushd %{wacom_tool}-%{wacom_tool_version}
%patch500 -p1 -b .fix-area-calib
%patch501 -p1 -b .xrandr-version-check
%patch502 -p1 -b .cintiq-without-xrandr
%patch503 -p1 -b .twinview-and-LED-support
%patch504 -p1 -b .pad-map-button
...
%{SOURCE3} --apply . %{SOURCE4}

make %{?_smp_mflags}
popd

%ifnarch s390 s390x
# Build gnome-wacom-properties
pushd %{wacom_tool}-%{wacom_tool_version}
autoreconf -f -i
...
install -m644 %{SOURCE2} $RPM_BUILD_ROOT%{_datadir}/polkit-1/actions
...
# Drop gnome-at-mobility
# http://bugzilla.redhat.com/771600
rm -rf $RPM_BUILD_ROOT%{_bindir}/gnome-at-mobility
...
%defattr(-, root, root)
%doc gnome-control-center-%{version}/AUTHORS gnome-control-center-%{version}/COPYING gnome-control-center-%{version}/NEWS gnome-control-center-%{version}/README
...
%ifnarch s390 s390x
%{_datadir}/gnome-wacom-properties/ui
%endif
...
%ifnarch s390 s390x
%{_bindir}/gnome-wacom-properties
%endif
%{_libdir}/*.so.*
%{_sysconfdir}/gconf/schemas/control-center.schemas
%{_sysconfdir}/gconf/schemas/gnome-control-center.schemas
...
* Fri Apr 27 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-36
- wacom: Make tablet configuration be per-machine
  Resolves: #816649

* Wed Mar 28 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-35
- wacom: fix unsetting touch strip button mapping
  Related: #805045

* Wed Mar 28 2012 Peter Hutterer <peter.hutterer@redhat.com> 2.28.1-34
- Update libwacom requires to new libwacom with Intuos5 support
  Related: #769465

* Thu Mar 22 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-33
- wacom: add support for NV-CONTROL extension (#805038)
- wacom: fix button mapping (#805045)
- wacom: add LED support from upstream (#805069)
  Resolves: #805038 #805045 #805069

* Thu Mar 08 2012 Ray Strode <rstrode@redhat.com> 2.28.1-32
- Obsolete wacomcpl
  Resolves: #801363

* Wed Mar  7 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-31
- wacom: add fix for Cintiq with non-XRandR driver
  Related: #769465

* Tue Mar  6 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-30
- wacom: add fix for XRandR version
- wacom: add fix for calibrator crash
  Related: #769465

* Mon Mar 05 2012 Ray Strode <rstrode@redhat.com> 2.28.1-29
- Put background.xml in ~/.config/gnome-control-center
  This makes it more easily admin relocatable
  Resolves: #524942

* Mon Mar 05 2012 Ray Strode <rstrode@redhat.com> 2.28.1-28
- Bring the shell back from the dead, in the -extra subpackage
  Resolves: #632680

* Mon Mar  5 2012 Olivier Fourdan <ofourdan@redhat.com> 2.28.1-27
- Add Wacom support
  Resolves: #769465

* Sun Mar 04 2012 Ray Strode <rstrode@redhat.com> 2.28.1-26
- Drop gnome-at-mobility
- Resolves: #771600
...
* Mon Aug 09 2010 Ray Strode <rstrode@redhat.com> 2.28.1-23
- Update translations again
  Resolves: #603417

* Tue Jul 20 2010 Ray Strode <rstrode@redhat.com> 2.28.1-22
- Drop control-center shell
  Resolves: #603287

* Wed Jun 30 2010 Ray Strode <rstrode@redhat.com> 2.28.1-21
- Fix ml translation
  Related: #576066

* Fri Jun 25 2010 Ray Strode <rstrode@redhat.com> 2.28.1-20
- Update translations
  Resolves: #576066

* Mon Jun 07 2010 Bastien Nocera <bnocera@redhat.com> 2.28.1-19
- Update translations
Resolves: rhbz#576066

* Tue Jun 01 2010 Bastien Nocera <bnocera@redhat.com> 2.28.1-18
- Add upstream patch to not disable tap-to-click for single-button
  touchpads
Related: rhbz#587863

* Mon May  3 2010 Matthias Clasen <mclasen@redhat.com> 2.28.1-16
- Update more translations
Resolves: #576066

* Mon Jan 18 2010 Matthias Clasen <mclasen@redhat.com> 2.28.1-15
- Fix Marathi translations

* Fri Jan 15 2010 Bastien Nocera <bnocera@redhat.com> 2.28.1-14
- Fix icon not appearing after succesful fingerprint capture
Related: rhbz#543948

* Tue Jan 05 2010 Ray Strode <rstrode@redhat.com> 2.28.1-13
Resolves: #552713
- Close fd leak

* Mon Jan  4 2010 Matthias Clasen <mclasen@redhat.com> - 2.28.1-12
- Improve handling of monitor aspect ratio
- Fix issues with  window manager keybindings

* Mon Dec  7 2009 Matthias Clasen <mclasen@redhat.com> 2.28.1-9
- Improve typing break locking for multiple monitors

* Tue Nov 24 2009 Matthias Clasen <mclasen@redhat.com> - 2.28.1-8
- Install PolicyKit .policy files in the right place (#540888)

* Wed Nov 18 2009 Peter Hutterer <peter.hutterer@redhat.com> 2.28.1-7
- Try again, cvs add failed for disable-touchpad-on-capabilities.patch for
  some unknown reason.

* Wed Nov 18 2009 Peter Hutterer <peter.hutterer@redhat.com> 2.28.1-6
